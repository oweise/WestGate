...
public class Xml {
    ...
    public Document load(HttpClient client, String url) throws UnsupportedEncodingException, WGException, IOException, HttpException, SAXException, DocumentException {
        ...
        HttpGet homePageMethod = new HttpGet(url);
        HttpResponse response = client.execute(homePageMethod);
        
        if (response.getStatusLine().getStatusCode() != 200) {
            throw new HttpException("HTTP Status Code is " + response.getStatusLine().getStatusCode());
        }
        
        SAXReader reader = new SAXReader();
        return reader.read(response.getEntity().getContent(), url);
