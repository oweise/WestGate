...
public class RestApplication extends ResourceConfig implements WGACoreEventListener, WGAConfigurationUpdateListener {
    ...
    private Map<String,DatabaseInfo> _databaseInfo = new HashMap<String, DatabaseInfo>();
    ...
    @Inject
    public RestApplication(WGACore wgaCore) throws WGException {
        
        // Context
        _wga = WGA.get(wgaCore);
        _encoding = _wga.getCore().getCharacterEncoding();
        
        // Init Jersey and swagger
        setClassLoader(_wga.server().getLibraryLoader());
        ...
        URLBuilder urlBuilder = _wga.urlBuilder(_wga.server().getBaseURL());
