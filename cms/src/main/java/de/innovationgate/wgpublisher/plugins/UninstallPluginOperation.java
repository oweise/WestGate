...
public class UninstallPluginOperation implements WorkspaceOperation {

    private PluginID _id;

    public UninstallPluginOperation(PluginID id) {
        _id = id;
    }
    ...
    public WGAPlugin perform(WGAPluginSet set, WGACore core, boolean workspaceOnly) throws PluginOperationException {
        ...
        if (plugin != null) {
            set.uninstallPlugin(plugin, workspaceOnly);
        }
        return plugin;
