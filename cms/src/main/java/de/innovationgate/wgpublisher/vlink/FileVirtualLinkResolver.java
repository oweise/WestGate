...
public class FileVirtualLinkResolver implements VirtualLinkResolver {
    ...
    public VirtualLinkTarget resolve(WGA wga, WGContent content) throws WGException {
        ...
        if (elements.size() < 2) {
            throw new WGIllegalDataException("Virtual link of type '" + content.getVirtualLinkType() + "' in " + content.getDocumentKey() + "' has too few elements (< 2)");
        }

        String containerName = (String) elements.get(0);
        String fileName = (String) elements.get(1);
        String dbKey = (elements.size() == 3 ? elements.get(2) : content.getDatabase().getDbReference());
        
        VirtualLinkTarget target = new VirtualLinkTarget(Type.ATTACHMENT);
        target.setContainerKey(containerName);
