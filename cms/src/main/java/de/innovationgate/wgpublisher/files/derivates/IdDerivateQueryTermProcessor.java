...
public class IdDerivateQueryTermProcessor implements DerivateQueryTermProcessor {

    @Override
    public float matchTermToFileAnnotations(WGFileAnnotations fileAnnotations, DerivateQueryTerm term, ClientHints clientHints) {
        if (fileAnnotations instanceof WGFileDerivateMetaData) {
            if (((WGFileDerivateMetaData) fileAnnotations).getId().equals(term.getValue())) {
                return 1;
            }
        }
