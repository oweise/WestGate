...
public class ErrorPageManager {
    ...
    public ErrorPageManager(PageContext pageContext) throws ServletException, IOException {
        ...
        WGACore core = CDI.current().select(WGACore.class).get();
        ...
        else if (core.getWgaConfiguration().isCustomErrorPageEnabled() && core.getErrorPage() != null) {
            response.reset();
            response.setContentType("text/html");
            request.getRequestDispatcher(core.getErrorPage()).forward(request, response);
            _displayDefaultErrorPage = false;
        }
    }
